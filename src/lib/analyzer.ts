...
function getBackAndForthDensity(messages: AnalysisMessage[]): number {
  ...
  // High density (lots of short back-and-forth) is a meeting signal
...
export function analyzeConversation(
  ...
  // Async-OK reduces score
  if (asyncHit) {
    const asyncReduction = Math.min(asyncHit.count * 1.5, 4);
    score -= asyncReduction;
  }
  ...
  // Participants: prefer those who actually spoke
  ...
  if (score >= 8 || uniqueParticipants >= 4) suggested_duration_mins = 45;
  else if (score >= 6 || uniqueParticipants >= 3) suggested_duration_mins = 30;
  ...
  // Agenda items from triggered signals only
